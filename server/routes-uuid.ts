...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/classes", requireAuth, async (req: Request, res: Response) => {
    try {
      const { name, subject, gradeLevel, schoolName } = req.body;
  ...
  app.post("/api/quiz-submissions", async (req, res) => {
    try {
      const { studentName, gradeLevel, classId, animalType, geniusType, animalGenius, answers, personalityType, learningStyle, scores } = req.body;
  ...
  app.post("/api/currency/give", requireAuth, async (req: any, res) => {
    try {
      ...
      res.json({ 
        success: true, 
        newBalance,
        message: `Gave ${amount} coins to ${student.name}` 
      });
    } catch (error) {
  ...
  app.post("/api/currency/take", requireAuth, async (req: any, res) => {
    try {
      ...
      res.json({ 
        success: true, 
        newBalance,
        message: `Took ${actualAmount} coins from ${student.name}` 
      });
    } catch (error) {
