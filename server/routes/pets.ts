...
async function flexiblePetAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (req.cookies?.student_session) {
    try {
      // Let requireStudentSession handle it
      return requireStudentSession(req, res, next);
    } catch (error) {
      // Continue to teacher auth check
    }
  }
  
  // Check for teacher auth
  if (req.user?.userId) {
    // Teacher is authenticated - they can interact with any pet
    // We'll need to determine which student's pet this is
    const petId = req.params.petId;
    if (petId) {
      // For teachers, we need to set a studentId for the interaction
 ...
 */
router.get('/my-pet', requireStudentSession, asyncWrapper(async (req, res, next) => {
 ...
 * Requires student authentication
 */
router.post('/purchase', requireStudentSession, storePurchaseLimiter, asyncWrapper(async (req, res, next) => {
 ...
 * Requires student authentication
 */
router.put('/:petId/position', requireStudentSession, asyncWrapper(async (req, res, next) => {
  const studentId = req.studentId!;
  const petId = req.params.petId;
 ...
 * Requires student authentication
 */
router.put('/:petId/rename', requireStudentSession, asyncWrapper(async (req, res, next) => {
  const studentId = req.studentId!;
  const petId = req.params.petId;
