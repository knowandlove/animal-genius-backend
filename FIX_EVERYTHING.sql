-- THIS SHOULD FIX EVERYTHING AND GET THE QUIZ WORKING AGAIN
-- Based on the working July 12 migration structure
...
BEGIN
  ...
  -- 4. Look up animal and genius types (handle both hyphen and underscore)
  -- First try with hyphen (as stored in DB)
  SELECT id INTO v_animal_type_id FROM public.animal_types 
  WHERE code = LOWER(REPLACE(calculated_animal, ' ', '-'));
  
  -- If not found, try with underscore
  IF NOT FOUND THEN
    SELECT id INTO v_animal_type_id FROM public.animal_types 
  ...
  IF NOT FOUND THEN
    SELECT id INTO v_animal_type_id FROM public.animal_types 
    WHERE LOWER(code) = LOWER(calculated_animal) OR LOWER(name) = LOWER(calculated_animal);
  END IF;
  
  -- If still not found, insert it with hyphen to match DB pattern
  IF NOT FOUND THEN
    INSERT INTO public.animal_types (code, name, personality_type, genius_type)
    VALUES (
      LOWER(REPLACE(calculated_animal, ' ', '-')), 
      calculated_animal, 
      calculated_animal, 
      calculated_genius
    )
  ...
  SELECT id INTO v_genius_type_id FROM public.genius_types 
  WHERE code = LOWER(calculated_genius);
  ...
  -- 8. NO STUDENT COUNT UPDATE (column doesn't exist)
  
  -- 9. Log the joining coins transaction (USE transaction_type NOT reason)
  INSERT INTO public.currency_transactions (
    student_id, amount, transaction_type, description, created_at
  ) VALUES (
    v_student_id, v_starting_balance, 'quiz_reward', 
    'Quiz completion reward', NOW()
  );
