...
-- Copy the function from the migration but change p_class_code to class_code
CREATE OR REPLACE FUNCTION submit_quiz_atomic(
  class_code TEXT,  -- Changed from p_class_code
  first_name TEXT,
  last_initial TEXT,
  grade TEXT,
  quiz_answers JSONB
...
BEGIN
  ...
  FROM classes
  WHERE class_code = class_code -- Fixed reference
    AND is_active = true
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION submit_quiz_atomic TO anon;
GRANT EXECUTE ON FUNCTION submit_quiz_atomic TO service_role;
EOF < /dev/null