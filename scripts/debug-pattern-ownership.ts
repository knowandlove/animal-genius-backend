...
import { students, studentInventory, storeItems, patterns, itemTypes } from '@shared/schema';
import { eq, and, or, like } from 'drizzle-orm';

async function debugPatternOwnership(passportCode: string) {
  ...
  try {
    ...
    console.log('\n🏠 Current room data:\n');
    const roomData = student.roomData as any;
    console.log(`Wall Pattern: ${roomData?.wallPattern || roomData?.wall?.value || 'none'}`);
    console.log(`Floor Pattern: ${roomData?.floorPattern || roomData?.floor?.value || 'none'}`);
...
if (!passportCode) {
  console.log('Usage: npx ts-node scripts/debug-pattern-ownership.ts PASSPORT_CODE');
  console.log('Example: npx ts-node scripts/debug-pattern-ownership.ts PAN-2BQ');
  process.exit(1);
}
