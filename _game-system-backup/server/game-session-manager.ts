...
import { redisGameSessionStore } from './lib/redis-game-session-store';
...
class GameSessionManager {
  ...
  async createGame(teacherId: number, settings: GameSettings, questions: GameQuestion[]): Promise<GameSession> {
    ...
    if (redisCache.isAvailable()) {
      await redisGameSessionStore.saveGame(game);
    }
  ...
  async getGameByCode(code: string): Promise<GameSession | null> {
    // Try local cache first
    const gameId = this.codeToGameId.get(code.toUpperCase());
    if (gameId) {
      const localGame = this.games.get(gameId);
      if (localGame) return localGame;
    }

    // Try Redis if available
    if (redisCache.isAvailable()) {
      const redisGameId = await redisGameSessionStore.getGameIdByCode(code);
      if (redisGameId) {
        const redisGame = await redisGameSessionStore.loadGame(redisGameId);
        if (redisGame) {
          // Cache locally
          this.games.set(redisGame.id, redisGame);
          this.codeToGameId.set(redisGame.code, redisGame.id);
          return redisGame;
        }
      }
    }
  ...
  async getGameById(gameId: string): Promise<GameSession | null> {
    ...
    // Try Redis if available
    if (redisCache.isAvailable()) {
      const redisGame = await redisGameSessionStore.loadGame(gameId);
      if (redisGame) {
        // Cache locally
        this.games.set(redisGame.id, redisGame);
        this.codeToGameId.set(redisGame.code, redisGame.id);
  ...
  async getGameByPlayerId(playerId: string): Promise<GameSession | null> {
    // Try local cache first
    ...
    // Try Redis if available
    if (redisCache.isAvailable()) {
      const redisGameId = await redisGameSessionStore.getGameIdByPlayerId(playerId);
      if (redisGameId) {
        return this.getGameById(redisGameId);
      }
    }
  ...
  async updatePlayerAnimal(gameId: string, playerId: string, animal: AnimalType): Promise<boolean> {
    ...
    // Update Redis if available
    if (redisCache.isAvailable()) {
      await redisGameSessionStore.updatePlayer(gameId, playerId, { animal });
    }

    return true;
  }

  async updatePlayerAvatar(gameId: string, playerId: string, avatar: AvatarCustomization): Promise<boolean> {
    const game = await this.getGameById(gameId);
    if (!game) return false;
    ...
    // Update Redis if available
    if (redisCache.isAvailable()) {
      await redisGameSessionStore.updatePlayer(gameId, playerId, { avatar });
    }
  ...
  async removePlayer(gameId: string, playerId: string): Promise<boolean> {
    ...
    // Update Redis if available
    if (redisCache.isAvailable() && deleted) {
      if (game.players.size === 0 && game.status === 'lobby') {
        await redisGameSessionStore.removeGame(game);
      } else {
        await redisGameSessionStore.saveGame(game);
      }
    }
  ...
  async disconnectPlayer(gameId: string, playerId: string): Promise<boolean> {
    const game = await this.getGameById(gameId);
    if (!game) return false;
    ...
    // Update Redis if available
    if (redisCache.isAvailable()) {
      await redisGameSessionStore.updatePlayer(gameId, playerId, { connected: false });
    }

    return true;
  }

  async reconnectPlayer(gameId: string, playerId: string): Promise<boolean> {
    const game = await this.getGameById(gameId);
    if (!game) return false;
    ...
    // Update Redis if available
    if (redisCache.isAvailable()) {
      await redisGameSessionStore.updatePlayer(gameId, playerId, { connected: true });
    }
