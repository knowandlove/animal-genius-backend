...
export class GameWebSocketServer {
  ...
  private setupWebSocketServer() {
    this.wss.on('connection', (ws: ExtendedWebSocket, request) => {
      ...
      if (this.activeConnections >= this.MAX_TOTAL_CONNECTIONS) {
        console.warn('WebSocket connection limit reached. Rejecting new connection.');
      ...
      if (ipConnections.size >= this.MAX_CONNECTIONS_PER_IP) {
        console.warn(`Too many connections from IP ${clientIP}. Rejecting.`);
        ws.close(1008, 'Too many connections from your location.');
        return;
      }
      ...
      console.log(`New WebSocket connection. Total active: ${this.activeConnections}`);
  ...
  private async handleJoinGame(ws: ExtendedWebSocket, data: JoinGameData) {
    ...
    // Player name is already validated and sanitized by validateWSMessage
    const sanitizedName = sanitizeString(data.playerName, 20);
    if (!sanitizedName) {
      if (process.env.NODE_ENV === 'development') {
  ...
  private handleDisconnect(ws: ExtendedWebSocket) {
    ...
    if (connectionId && clientIP) {
      ...
      this.activeConnections--;
      console.log(`WebSocket disconnected. Total active: ${this.activeConnections}`);
    }
    ...
    if (ws.isTeacher) {
      // Teacher disconnected - clean up game timer to prevent memory leak
      if (process.env.NODE_ENV === 'development') {
        console.log('Teacher disconnected from game:', ws.gameId);
      }
      // Clear the question timer if teacher disconnects
      this.clearQuestionTimer(ws.gameId);
    } else {
