...
-- This fixes the critical bug where quiz_submissions table was empty
...
BEGIN
  ...
  -- 5. Calculate MBTI type and learning style properly
  -- Extract MBTI type from the quiz answers (simplified version - ideally use the same logic as calculate_animal_type)
  -- For now, we'll derive it from the animal type since calculate_animal_type works correctly
  ...
  -- Calculate learning style from VARK questions (questions 6, 11, 17-24)
  -- For now, simplified calculation - count VARK answers
  DECLARE
    ...
    answer_text TEXT;
  BEGIN
    -- Count VARK question responses (questions 6 and 11 are VARK in the schema)
    -- Question 6 (index 5)
  ...
  -- 6. Get animal and genius type IDs
  ...
  -- 7. Create student record with MBTI type as personality_type
  ...
  -- 8. **THE CRITICAL FIX**: Create quiz_submission record
  INSERT INTO public.quiz_submissions (
    student_id, animal_type_id, genius_type_id, 
    answers, score, completed_at, created_at
  ) VALUES (
    v_student_id, v_animal_type_id, v_genius_type_id,
    quiz_answers, v_score, NOW(), NOW()
  ) RETURNING id INTO v_submission_id;
  
  -- 9. Update student count
  UPDATE public.classes 
  SET number_of_students = COALESCE(number_of_students, 0) + 1
  WHERE id = v_class_id;
  
  -- 10. Log currency transaction with CORRECT type for constraint
  ...
  -- 11. Return success with all data
