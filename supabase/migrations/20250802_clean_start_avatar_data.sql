-- Clean start: Remove test data and ensure avatar_data is properly initialized
-- Since we have no live students, this is the cleanest approach
...
-- Reset student count in classes
UPDATE public.classes SET student_count = 0;

-- 2. Add DEFAULT constraint for avatar_data to ensure all future records have correct structure
ALTER TABLE public.students
ALTER COLUMN avatar_data SET DEFAULT jsonb_build_object(
...
-- 3. Update the create_student_from_quiz_with_results function to initialize avatar_data
-- (This ensures the function also sets the correct structure, belt and suspenders approach)
...
BEGIN
  ...
  -- 4. Look up animal and genius types
  SELECT id INTO v_animal_type_id FROM public.animal_types WHERE code = calculated_animal;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Invalid animal type: %', calculated_animal;
  END IF;
  
  SELECT id INTO v_genius_type_id FROM public.genius_types WHERE code = calculated_genius;
  IF NOT FOUND THEN
  ...
  -- Note: We explicitly set avatar_data even though we have a DEFAULT, for clarity
  ...
  -- 9. Log the joining coins transaction
...
-- 4. Add documentation
COMMENT ON COLUMN students.avatar_data IS 'Avatar customization data. Structure: {colors: {hasCustomized: boolean, primaryColor: string|null, secondaryColor: string|null}}. May be extended with equipped items in future.';

-- 5. Also clean up any orphaned auth users from testing
-- This is optional but helps keep the auth.users table clean
DO $$
BEGIN
  -- Delete auth users that don't have corresponding students
  DELETE FROM auth.users
  WHERE id NOT IN (SELECT user_id FROM public.students WHERE user_id IS NOT NULL)
  AND email LIKE '%@anonymous.local'
  AND created_at < NOW() - INTERVAL '1 hour'; -- Only delete test users older than 1 hour
END $$;